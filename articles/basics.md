...
extents, the `resolution` argument of
...
unnecessarily large datasets. The `resolution` argument is expressed in
arc-minutes. The highest resolutions are obtained with
`resolution = 0.25` (0.25 arc-minutes, equivalent to 15 arc-seconds).
...
and
[`get_noaa()`](https://besibo.github.io/marmap2/reference/get_noaa.md)
provide a `keep` argument to store a local copy of the downloaded data
...
can be used to print useful information about the imported data.
...
This is preferable to sending repeated queries for the same area to a
remote server. As in get_noaa(), the `resolution` argument is expressed
in arc-minutes, and the `method` argument specifies how values in the
coarser grid are derived from the finer grid: nearest-cell value
...
and
[`get_gebco()`](https://besibo.github.io/marmap2/reference/get_gebco.md)
return a tibble by default. `class = "bathy"` can be used to return a
...
is meant for data inspection just after downloading or transforming a
grid.
...
![](basics_files/figure-html/quickplot-1.png)
...
![](basics_files/figure-html/quickplot-2.png)
...
![](basics_files/figure-html/quickplot-3.png)

The same maps can be built layer by layer.
[`geom_bathy()`](https://besibo.github.io/marmap2/reference/geom_bathy.md)
draws the bathymetric raster,
[`geom_coastline()`](https://besibo.github.io/marmap2/reference/geom_coastline.md)
draws the 0 m contour, and
[`geom_contour()`](https://ggplot2.tidyverse.org/reference/geom_contour.html)
can add isobaths and elevation contours. The manual appraoch allows for
finer control over the appearance of each layer and the overall map. The
example below shows a detailed map of the Alboran Sea, with bathymetry,
coastline, isobaths every hundred meters and elevation contours every
500 meters.
...
![](basics_files/figure-html/detailed-alboran-map-1.png)
...
for the full list), a vector of colours, a palette function, or a single
colour. This makes it possible to use the palettes shipped with
`marmap2`, palettes from other R packages, base R HCL palettes, or
hand-made gradients. The only important rule is to choose ocean and land
palettes that work together visually and remain ordered from deep to
shallow water, and from low to high elevation.
...
![](basics_files/figure-html/palette-built-in-1.png)

The same function can use colour-generating functions. The next example
uses base R HCL palettes through
...
![](basics_files/figure-html/palette-hcl-1.png)
...
![](basics_files/figure-html/palette-external-1.png)
...
![](basics_files/figure-html/palette-manual-1.png)
...
![](basics_files/figure-html/palette-mediterranean-truncate-1.png)
...
![](basics_files/figure-html/palette-alboran-truncate-1.png)
...
![](basics_files/figure-html/palette-alboran-rescale-1.png)

For a contour-only map, you can use
[`scale_colour_bathy()`](https://besibo.github.io/marmap2/reference/scale_fill_bathy.md)
with the levels computed by
[`geom_contour()`](https://ggplot2.tidyverse.org/reference/geom_contour.html)
to color code isobaths or elevation contours. In the example below,
[`geom_bathy()`](https://besibo.github.io/marmap2/reference/geom_bathy.md)
is still used to draw the geographic background and land mask, but the
ocean fill is made transparent. The depth information is carried only by
the contour lines.
...
![](basics_files/figure-html/contour-only-1.png)

Here,
[`scale_fill_bathy()`](https://besibo.github.io/marmap2/reference/scale_fill_bathy.md)
controls the background only: ocean cells are transparent and land cells
are grey.
[`scale_colour_bathy()`](https://besibo.github.io/marmap2/reference/scale_fill_bathy.md)
controls the isobaths. Setting `palette_land = NULL` tells the colour
scale that only ocean values should be mapped, so shallow negative
depths are not treated as land colours. `show.legend = FALSE` in
[`geom_bathy()`](https://besibo.github.io/marmap2/reference/geom_bathy.md)
avoids displaying a second legend for the transparent background layer.
...
`antimeridian = TRUE` in
[`geom_bathy()`](https://besibo.github.io/marmap2/reference/geom_bathy.md).
The x axis then remains readable, with labels expressed as east and west
longitudes.
...
![](basics_files/figure-html/antimeridian-plot-1.png)
...
![](basics_files/figure-html/projection-map-1.png)
...
rather than as a regular grid.
[`griddify()`](https://besibo.github.io/marmap2/reference/griddify.md)
bins these points into a regular grid and, optionally, fills empty cells
...
![](basics_files/figure-html/griddify-1.png)

The points show the original sampling pattern. The background grid is
the regularised bathymetry returned by
[`griddify()`](https://besibo.github.io/marmap2/reference/griddify.md)
and can be passed to all the same plotting and conversion functions as
imported data.
...
Use
[`tbl_to_bathy()`](https://besibo.github.io/marmap2/reference/bathy_to_tbl.md)
when older marmap code expects a `bathy` matrix. Use
[`as_xyz()`](https://besibo.github.io/marmap2/reference/as_xyz.md) for
external software that expects a plain three-column xyz table. Use
[`as_sf()`](https://besibo.github.io/marmap2/reference/as_sf.md) and
[`as_spatraster()`](https://besibo.github.io/marmap2/reference/as_spatraster.md)
to work with the wider R spatial ecosystem.
