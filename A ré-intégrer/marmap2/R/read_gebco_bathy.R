...
#'
#' @details
#' \code{read_gebco_bathy} reads a 30 arcseconds or 1 arcminute bathymetry file downloaded from the GEBCO (General Bathymetric Chart of the Oceans) website (British Oceanographic Data Center). The website allows the download of bathymetric data in the netCDF format. \code{read_gebco_bathy} uses the \code{ncdf4} package to load the data into R, and parses it into an object of class \code{bathy}.
#'
#' Data can be downloaded from the 30 arcseconds database (GEBCO_08) or the 1 arcminute database (GEBCO_1min, the default). A third database type, GEBCO_08 SID, is available from the website. This database includes a source identifier specifying which grid cells have depth information based on soundings ; it does not include bathymetry or topography data. \code{read_gebco_bathy} can read this type of database when \code{sid} is set to \code{TRUE}. Then only the SID information will be included in the object of class \code{bathy}. Therefore, to display a map with both the bathymetry and the SID information, you will have to download both datasets from GEBCO, and import and plot both independently.
#'
...
#'
#' @return
#' The output of \code{read_gebco_bathy} is a matrix of class \code{bathy}, which dimensions depends on the resolution specified (one-minute, the original GEBCO resolution, is the default). The class \code{bathy} has its own methods for summarizing and ploting the data.
#'
#' @references
...
#'
#' @seealso
#' \code{\link{get_noaa}}, \code{\link{read_bathy}}, \code{\link{plot_bathy}}
#'
#' @examples
#' \dontrun{
...
#' }
#' @export
read_gebco_bathy <- function(file, resolution=1, sid=FALSE){
	...
	if(resolution<1 | !is.wholenumber(resolution)) stop(message) else {
		...
		# rownames(mat) <- xdim
		# colnames(mat) <- ydim
		mat <- check_bathy(mat[,ncol(mat):1])
