...
#' @rdname get_depth
#' @usage
#' get_depth(mat, x, y=NULL, locator=TRUE, distance=FALSE, \dots)
...
#'
#' @details
#' \code{get_depth} allows the user to get depth data by clicking on a map created with \code{plot_bathy} or by providing coordinates of points of interest. This function uses the \code{locator} function (\code{graphics} package); after creating a map with \code{plot_bathy}, the user can click on the map once or several times (if \code{locator=TRUE}), press the Escape button, and get the depth of those locations in a three-coumn data.frame (longitude, latitude and depth). Alternatively, when \code{locator=FALSE}, the user can submit a list of longitudes and latitudes, a two-column matrix or data.frame of longitudes and latitudes (as input for \code{x}), or one vector of longitudes (\code{x}) and one vector of latitudes (\code{y}). The non-interactive mode is well suited to get depth information for each point provided by GPS tracking devices. While \code{\link{get_transect}} gets every single depth value available in the bathymetric matrix between two points along a user-defined transect, \code{get_depth} only provides depth data for the specific points provided as input by the user.
#'
#' @return
...
#'
#' @seealso
#' \code{\link{path_profile}}, \code{\link{get_transect}}, \code{\link{read_bathy}}, \code{\link{summary_bathy}}, \code{\link{subset_bathy}}, \code{\link{nw.atlantic}}
#'
#' @examples
...
#' @export
get_depth <- function(mat, x, y=NULL, locator=TRUE, distance=FALSE, ...){
