...
#' Computes least cost distances between two or more locations
#'
#' @rdname least_cost_distance
#' @usage
#' least_cost_distance(trans, loc, res = c("dist", "path"), meters = FALSE, round = 0)
...
#' @param meters logical. When \code{res = "dist"}, should the results be returned in kilometers (default) or in meters.
#' @param round integer indicating the number of decimal places to be used for printing results when \code{res = "dist"}.
#'
#' @details
#' \code{least_cost_distance} computes least cost distances between 2 or more locations. This function relies on the package \code{gdistance} (van Etten, 2011. \url{https://CRAN.R-project.org/package=gdistance}) and on the \code{transition_matrix} function to define a range of depths where the paths are possible.
#'
#' @return
...
#' }
#' @export
least_cost_distance <- function(trans, loc, res = c("dist", "path"), meters = FALSE, round = 0) {
	...
	loc.depth <- get_depth(bathymetry, x = loc, locator = FALSE)

	if (any(loc.depth[,3] > min.depth) | any(loc.depth[,3] < max.depth)) print(loc.depth)
	if (any(loc.depth[,3] > min.depth) | any(loc.depth[,3] < max.depth)) warning(paste("One or more points are located outside of the [",min.depth, ";", max.depth,"] depth range. You will get unrealistically huge distances unless you either increase the range of possible depths in transition_matrix() or you move the problematic points in a spot where their depths fall within the [",min.depth, ";", max.depth,"] depth range.\nYou can use get_depth() to determine the depth of any point on a bathymetric map", sep=""))
