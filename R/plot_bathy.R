...
#'
#' @rdname plot_bathy
#' @param x bathymetric data matrix of class \code{bathy}, imported using \code{read_bathy}
...
#'
#' @seealso
#' \code{\link{read_bathy}}, \code{\link{summary_bathy}}, \code{\link{nw.atlantic}}, \code{\link{metallo}}
#'
#' @examples
...
plot.bathy <- function(x, image=FALSE, bpal=NULL, land=FALSE, deepest.isobath, shallowest.isobath, step, n=20, lwd=1, lty=1, col="black", default.col="white", drawlabels = FALSE, xlab="Longitude", ylab="Latitude", asp=1, ...){
	...
	if (image == TRUE) {
		...
		if (is.list(bpal))	bpal <- palette_bathy(mat, layers = bpal, land=land, default.col=default.col)
