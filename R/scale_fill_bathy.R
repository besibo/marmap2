...
bathy_palette_specs <- function() {
  list(
    ocean_blues = list(
      colours = c(
        "#00366C", "#005D9A", "#2C86CA", "#79ABE2",
        "#ADCCF6", "#D8E9FF", "#F9F9F9"
      )
    ),
    ocean_teal = list(
      colours = c(
        "#14505C", "#226B70", "#348781", "#4CA38F",
        "#6BBE99", "#9AD4A8", "#C7E5BE"
      )
    ),
    ocean_deep = list(
      colours = c(
        "#070707", "#392243", "#404B7B", "#007FA8",
        "#00AFB2", "#87D4BC", "#E0F7E1"
      )
    ),
    ocean_mako = list(
      colours = c(
        "#0B0B0C", "#22283D", "#244B6B", "#1C7191",
        "#2499A2", "#6BC4B1", "#D7F2DB"
      )
    ),
    ocean_ice = list(
      colours = c(
        "#10243E", "#1E4768", "#356D8B", "#5A93A9",
        "#8AB9C6", "#C3DEE4", "#F2F8F9"
      )
    ),
    ocean_mint = list(
      colours = c(
        "#163E48", "#245C60", "#367A72", "#509984",
        "#78B896", "#AAD5B2", "#E0EFD5"
      )
    ),
    land_hcl = list(
      colours = c(
        "#F7F3DF", "#DEC79D", "#BD7864", "#A15558",
        "#7A374C", "#541C3A", "#E2E2E2"
      )
    ),
    land_earth = list(
      colours = c(
        "#F7F3DF", "#E6D2A3", "#C8AA6E", "#9B7A4D",
        "#6F5946", "#4A3F3C", "#E2E2E2"
      )
    ),
    land_green = list(
      colours = c(
        "#FEFEE3", "#EAF5CA", "#C2E2A2", "#85C876",
        "#40A45D", "#0A764E", "#E2E2E2"
      )
    ),
    land_sand = list(
      colours = c(
        "#F8F1D7", "#E9D5A5", "#D2B476", "#B58E55",
        "#876843", "#594739", "#E6E3DD"
      )
    ),
    land_olive = list(
      colours = c(
        "#F7F3D2", "#D9DEA3", "#AFC06F", "#819849",
        "#5F7038", "#444D32", "#DFDFD8"
      )
    ),
    land_brown = list(
      colours = c(
        "#F4EAD0", "#D9BD8A", "#B9905B", "#8D6844",
        "#604935", "#3B332E", "#DFDFDF"
      )
    )
  )
}
...
bathy_scale_spec <- function(palette_ocean, palette_land) {
  ...
  if (is.null(ocean) && is.null(land)) {
    stop("At least one of palette_ocean or palette_land must be provided.", call. = FALSE)
  }
...
bathy_palette_from_input <- function(palette, n = 256) {
  ...
  if (!is.character(palette) || length(palette) < 1 || anyNA(palette)) {
    stop("palette must be a palette name, colour, colour vector, or function.", call. = FALSE)
  }
  ...
  bathy_palette_from_colour_vector(palette, type = if (length(palette) == 1) "colour" else "vector")
}

bathy_palette_from_colour_vector <- function(colours, type) {
  if (!is.character(colours) || length(colours) < 1 || anyNA(colours) || !all(bathy_is_colour(colours))) {
...
bathy_user_limits <- function(limits, coverage) {
  ...
  if (!is.numeric(limits) || length(limits) != 2 || anyNA(limits) || any(!is.finite(limits))) {
    stop("limits must be a finite numeric vector of length two.", call. = FALSE)
  }
  limits <- sort(limits)

  if (identical(coverage, "both") && !(limits[1] < 0 && limits[2] > 0)) {
    stop("Using both ocean and land palettes requires limits spanning zero.", call. = FALSE)
  }
...
bathy_scale_values <- function(spec, limits, mode) {
  if (identical(spec$coverage, "ocean")) {
    ...
    colours <- c(spec$ocean$colours, bathy_constant_side_colour(spec$land, "grey80"))
    ...
    colours <- c(bathy_constant_side_colour(spec$ocean, "#D8EEF3"), spec$land$colours)
...
bathy_rescale_coverage <- function(x, from, coverage) {
  from <- sort(from)
  if (!is.numeric(from) || length(from) != 2 || anyNA(from) || diff(from) == 0) {
