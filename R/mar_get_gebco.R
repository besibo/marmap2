...
#'
#' @details
#' \code{mar_get_gebco} uses the workflow implemented by the official GEBCO
...
#' consistency with \code{\link{mar_get_noaa}}, but it is not sent to the GEBCO
...
#'   where \code{mar_get_gebco} looks for an already downloaded matching csv
...
#'
#' @seealso
#' \code{\link{mar_get_noaa}}, \code{\link{mar_read_gebco_bathy}},
#' \code{\link{mar_read_bathy}}, \code{\link{mar_as_bathy}}
#'
#' @examples
#' \dontrun{
...
mar_get_gebco <- function(
  ...
  if (file.exists(csv_file)) {
    ...
    return(mar_read_bathy(csv_file, header = TRUE))
  }
