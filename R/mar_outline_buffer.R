...
#'
#' @description
#' Get a buffer (i.e. a non-circular buffer as produced by \code{mar_combine_buffers()}) in a format suitable for plotting its outline. \code{mar_outline_buffer()} replaces any \code{NA} values in a \code{buffer} or \code{bathy} object by 0 and non-\code{NA} values by -1.
#'
#' @rdname mar_outline_buffer
#' @usage
#' mar_outline_buffer(buffer)
...
#'
#' @details
#' This function is essentially used to prepare a composite buffer for plotting its outline on a bathymetric map. Plotting a single circular buffer should be done using the \code{mar_plot_buffer()} function since it offers a more straightforward method for plotting and much smoother outlines, especially for low-resolution bathymetries.
#'
#' @return
...
#' }
#' @export
#' @aliases outline.buffer
mar_outline_buffer <- function(buffer) {
...
#' @rdname mar_outline_buffer
#' @export
outline.buffer <- mar_outline_buffer
