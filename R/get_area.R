...
#' @rdname get_area
#' @usage
#' get_area(mat, level.inf, level.sup=0, xlim=NULL, ylim=NULL)
#' @param mat bathymetric data matrix of class \code{bathy}, imported using \code{read_bathy} (no default)
...
#'
#' @details
#' \code{get_area} calculates the projected surface area of specific depth layers (e.g. upper bathyal, lower bathyal), the projected plane being the ocean surface. The resolution of \code{get_area} depends on the resolution of the input bathymetric data. \code{xlim} and \code{ylim} can be used to restrict the area of interest. Area calculation is based on \code{areaPolygon} of package \code{geosphere} (using an average Earth radius of 6,371 km).
#'
#' @return
...
#'
#' @seealso
#' \code{\link{plot_area}}, \code{\link{plot_bathy}}, \code{\link{contour}}, \code{\link[geosphere]{areaPolygon}}
#'
#' @examples
...
#' 	plot(hawaii, lwd=0.2)
#'
#' 	mesopelagic <- get_area(hawaii, level.inf=-1000, level.sup=-200)
#' 	bathyal <- get_area(hawaii, level.inf=-4000, level.sup=-1000)
#' 	abyssal <- get_area(hawaii, level.inf=min(hawaii), level.sup=-4000)
#'
#' 	col.meso <- rgb(0.3, 0, 0.7, 0.3)
#' 	col.bath <- rgb(0.7, 0, 0, 0.3)
#' 	col.abys <- rgb(0.7, 0.7, 0.3, 0.3)
#'
#' 	plot_area(mesopelagic, col = col.meso)
...
#' 	plot(hawaii, lwd=0.2)
#'
#' 	mesopelagic <- get_area(hawaii, xlim=c(-161.4,-159), ylim=c(21,23),
#' 	                        level.inf=-1000, level.sup=-200)
#' 	bathyal <- get_area(hawaii, xlim=c(-161.4,-159), ylim=c(21,23),
#' 	                        level.inf=-4000, level.sup=-1000)
#' 	abyssal <- get_area(hawaii, xlim=c(-161.4,-159), ylim=c(21,23),
...
#' 		pt.bg=c(col.meso,col.bath,col.abys))
#' @export
get_area <- function(mat, level.inf, level.sup=0, xlim=NULL, ylim=NULL) {
