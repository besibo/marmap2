...
#'
#' @details
#' \code{mar_palette_bathy} allows the production of color palettes for specified bathymetric and/or topographic layers. The \code{layers} argument must be a list of vectors. Each vector corresponds to a bathymetry/topography layer (for example, one layer for bathymetry and one layer for topography). The first and second elements of the vector are the minimum and maximum bathymetry/topography, respectively. The other elements of the vector (3, onward) correspond to colors (see example below). \code{mar_palette_bathy} is called internally by \code{mar_plot_bathy} when the \code{image} argument is set to \code{TRUE}.
#'
#' @return
...
mar_palette_bathy <- function(mat, layers, land=FALSE, default.col="white"){
...
#' @rdname mar_palette_bathy
#' @export
palette.bathy <- mar_palette_bathy
