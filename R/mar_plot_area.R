...
#'
#' @rdname mar_plot_area
#' @usage
#' mar_plot_area(area, col)
#' @param area a list of 4 elements as produced by \code{\link{mar_get_area}}.
#' @param col color of the projected surface area on the map.
#'
#' @author
#' Benoit Simon-Bouhet
#'
#' @seealso
#' \code{\link{mar_get_area}}, \code{\link{mar_plot_bathy}}, \code{\link[geosphere]{areaPolygon}}
#'
#' @examples
...
#' buf <- mar_create_buffer(florida, loc, radius=1.8)
#'
#' # Get the surface within the buffer for several depth slices
#' surf1 <- mar_get_area(buf, level.inf=-200, level.sup=-1)
#' surf2 <- mar_get_area(buf, level.inf=-800, level.sup=-200)
#' surf3 <- mar_get_area(buf, level.inf=-3000, level.sup=-800)
#'
...
#'
#' mar_plot_area(surf1, col = col.surf1)
#' mar_plot_area(surf2, col = col.surf2)
#' mar_plot_area(surf3, col = col.surf3)
#' plot(buf, lwd = 0.7)
...
#' @export
#' @aliases plotArea
mar_plot_area <- function(area, col) {
  if (!is.list(area)) stop("'area' must be a ist of 4 elements as produced by 'mar_get_area()'")
