...
#'
#' @rdname mar_path_profile
#' @usage
#' mar_path_profile(path,bathy,plot=FALSE, \dots)
#' @param path 2-columns matrix of longitude and latitude as obtained from \code{mar_least_cost_distance} with argument \code{dist=TRUE}.
#' @param bathy bathymetric data matrix of class \code{bathy}.
#' @param plot logical. Should the depth profile be plotted?
#' @param \dots when \code{plot=TRUE}, other arguments to be passed to \code{mar_plot_profile}, such as \link{graphical parameters} (see \code{\link{par}} and \code{\link{mar_plot_profile}}).
#'
#' @return
...
mar_path_profile <- function(path,bathy,plot=FALSE,...) {
  ...
  if (!is(bathy, "bathy")) stop("Object bathy is not of class bathy")
	...
	for (i in 1:(nrow(path)-1)) {
		df <- mar_get_transect(mat=bathy, x1=path[i,1], y1=path[i,2],x2=path[i+1,1],y2=path[i+1,2],distance=TRUE)
		# df <- df[-1,]
