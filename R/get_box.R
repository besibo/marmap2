...
#'
#' @description
#' \code{get_box} gets depth information of a belt transect of width \code{width} around a transect defined by two points on a bathymetric map.
#'
#' @rdname get_box
#' @usage
#' get_box(bathy,x1,x2,y1,y2,width,locator=FALSE,ratio=FALSE, \dots)
...
#' @param locator Logical. Whether to choose transect bounds interactively with a map or not. When \code{FALSE} (default), a bathymetric map  (\code{plot_bathy(bathy,image=TRUE)}) is automatically plotted and the position of the belt transect is added.
...
#'
#' @details
#' \code{get_box} allows the user to get depth data for a rectangle area of the map around an approximate linear transect (belt transect). Both the position and size of the belt transect are user defined. The position of the transect can be specified either by inputing start and stop coordinates, or by clicking on a map created with \code{plot_bathy}. In its interactive mode, this function uses the \code{locator} function (\code{\link{graphics}} package) to retrieve and plot the coordinates of the selected transect. The argument \code{width} allows the user to specify the width of the belt transect in degrees.
#'
#' @return
...
#'
#' @seealso
#' \code{\link{plot_bathy}}, \code{\link{get_transect}}, \code{\link{get_depth}}
#'
#' @examples
...
#' @export
get_box <- function(bathy,x1,x2,y1,y2,width,locator=FALSE,ratio=FALSE,...) {
  ...
  if (!is(bathy, "bathy")) stop("The matrix provided is not of class bathy")
	...
	if (p1x==p4x | p1y==p4y) {
		...
		coord1 <- diag_bathy(bathy[p1x:p4x,p1y:p4y],coord=TRUE)
	}
	...
	if (p2x==p3x | p2y==p3y) {
		...
		coord2 <- diag_bathy(bathy[p2x:p3x,p2y:p3y],coord=TRUE)
	}
	...
	out <- apply(tr,1,function(x) get_transect(x1=x[1],x2=x[3],y1=x[2],y2=x[4],mat=bathy,distance=TRUE))
