...
#'
#' @description
#' Creates a transition object to be used by \code{least_cost_distance} to compute least cost distances between locations.
#'
...
#'
#' @details
#' \code{transition_matrix} creates a transition object usable by \code{least_cost_distance} to computes least cost distances between a set of locations. \code{transition_matrix} rely on the function \code{raster} from package \code{raster} (Hijmans & van Etten, 2012. \url{https://CRAN.R-project.org/package=raster}) and on \code{transition} from package \code{gdistance} (van Etten, 2011. \url{https://CRAN.R-project.org/package=gdistance}).
#'
#' The transition object contains the probability of transition from one cell of a bathymetric grid to adjacent cells and depends on user defined parameters. \code{transition_matrix} is especially usefull when least cost distances need to be calculated between several locations at sea. The default values for \code{min.depth} and \code{max.depth} ensure that the path computed by \code{least_cost_distance} will be the shortest path possible at sea avoiding land masses. The path can be constrained to a given depth range by setting manually \code{min.depth} and \code{max.depth}. For instance, it is possible to limit the possible paths to the continental shelf by setting \code{max.depth=-200}. Inaccuracies of the bathymetric data can occasionally result in paths crossing land masses. Setting \code{min.depth} to low negative values (e.g. -10 meters) can limit this problem.
#'
#' \code{transition_matrix} takes also advantage of the function \code{geoCorrection} from package \code{gdistance} (van Etten, 2012. \url{https://CRAN.R-project.org/package=gdistance}) to take into account map distortions over large areas.
#'
#' @return
#' A transition object.
#'
...
#'
#' @seealso
#' \code{\link{least_cost_distance}}, \code{\link{hawaii}}
#'
#' @examples
...
#' }
#' @export
transition_matrix <- function(bathy,min.depth=0,max.depth=NULL) {
