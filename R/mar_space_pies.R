...
#'
#' @details
#' \code{mar_space_pies} tries to position piecharts on a map while avoiding overlap between them. The function heavily relies on two other functions. \code{floating.pie} from package plotrix is used to draw individual piecharts. \code{floating.pie} treats one pie at a time; \code{mar_space_pies} can handle one or multiple pies by looping \code{floating.pie}. \code{pointLabels} from package maptools was modified to find the best placement for the pies, given their size and distance from their anchor point. \code{pointLabels} was originally meant to automatically place text labels, not objects; the modified version contained in \code{mar_space_pies} uses the coordinates chosen by \code{pointLabels} for text. The algorithm used is simulating annealing (SANN). You can get a different result each time you run \code{mar_space_pies}, because \code{pointLabel} finds one good solution out of many. If you are not satisfied by the solution, you can try running the function again.
#'
...
#'
#' @seealso
#' \code{\link{mar_plot_bathy}}, \code{plotrix::floating.pie}, \code{maptools::pointLabel}
#'
#' @examples
