...
#'
#' @rdname mar_subset_bathy
#' @usage
#' mar_subset_bathy(mat, x, y=NULL, locator=TRUE, \dots)
...
#'
#' @details
#' \code{mar_subset_bathy} allows the user to generate new \code{bathy} objects by extracting data from larger \code{bathy} objects. The extraction of bathymetric data can be done interactively by clicking on a bathymetric map, or by providing longitudes and latitudes for the boundaries for the new \code{bathy} object. If two data points are provided, a rectangular area is selected. If more than two points are provided, a polygon is defined by linking the points and the bathymetic data is extracted within the polygon only. \code{mar_subset_bathy} relies on the \code{point.in.polygon} function from package \code{sp} to identify which points of the initial bathy matrix lie witin the boundaries of the user-defined polygon.
#'
#' @return
...
#'
#' @seealso
#' \code{\link{mar_plot_bathy}}, \code{\link{mar_get_depth}}, \code{\link{mar_summary_bathy}}, \code{\link{aleutians}}
#'
#' @examples
...
#' @export
#' @aliases subsetBathy
mar_subset_bathy <- function(mat, x, y=NULL, locator=TRUE, ...) {
	...
	if (locator == FALSE) {
		...
		if (nrow(out)==2) {
			...
			new.bathy <- mar_check_bathy(new.bathy)
		...
		if (nrow(out)>2) {
			...
			xyz <- as.matrix(mar_as_xyz(new.bathy))
			out <- as.matrix(out)
			inside <- sp::point.in.polygon(xyz[,1],xyz[,2],out[,1],out[,2])
			xyz[inside==0,3] <- NA
			new.bathy <- mar_as_bathy(xyz)
		}
