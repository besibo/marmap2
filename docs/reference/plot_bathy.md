...
## Arguments
  ...
  - bpal:
    ...
    if image is `TRUE`, either `NULL` (default: a simple blue color
    palette is used), a vector of colors, or a list of depth bounds and
    colors (see below)

  - land:
    
    whether or not to use topographic data that may be available in the
    `bathy` dataset (default is `FALSE`)
  ...
  - n:
    ...
    if the user does not specify the range within which isobaths should
    be plotted, about `n` isobaths are automatically plotted within the
    depth range of the `bathy` matrix (default is 20).
  ...
  - default.col:
    ...
    if image is `TRUE`, a color for the area of the matrix not bracketed
    by the list supplied to bpal (see below; default is `white`)

  - drawlabels:
    
    whether or not to plot isobath depth as a label (default is
    `FALSE`); may contain several elements
  ...
  - ...:
    ...
    Other arguments to be passed either to `countour` (default) or to
    `image` when argument `image=TRUE`.
...
`plot_bathy` uses the base `contour` and `image` functions. If a vector
of isobath characteristics is provided, different types of isobaths can
be added to the same plot using a single call of `plot_bathy` (see
examples)

If `image=TRUE`, the user has three choices for colors: (1) bpal can be
set to `NULL`, in which case a default blue color palette is generated;
(2) colors can be user-defined as in example 4, in which case the
palette can be generated with function `colorRampPalette` (colors are
then supplied as a vector to `plot_bathy`) ; (3) colors can be
constrained to bathymetry- and/or topography. In this last case, a list
of vectors is supplied to `plot_bathy` (example 7): each vector
corresponds to a bathymetry/topography layer (for example, one layer for
bathymetry and one layer for topography). The first and second elements
of the vector are the minimum and maximum bathymetry/topography,
respectively. The other elements of the vector (3, onward) correspond to
colors (see example 7).

## References

Eric Pante, Benoit Simon-Bouhet (2013) marmap: A Package for Importing,
Plotting and Analyzing Bathymetric and Topographic Data in R. PLoS ONE
8(9): e73051. doi:10.1371/journal.pone.0073051.
...
## Example 4: make a pretty custom color ramp
  ...
  scale_bathy(atl, deg=3, x="bottomleft", inset=5)
...
## Example 7: provide a list of depths and colors to argument bpal to finely tune palette
  # check out ?palette_bathy to see details on how the palette is handled
