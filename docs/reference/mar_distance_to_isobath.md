...
``` r
mar_distance_to_isobath(mat, x, y=NULL, isobath=0, locator=FALSE, ...)
...
`mar_distance_to_isobath` allows the user to compute the shortest great
circle distance between a set of points (selected interactively on a map
or not) and a user-defined isobath. `mar_distance_to_isobath` takes
...
`mar_lines_gc`, `mar_least_cost_distance`
