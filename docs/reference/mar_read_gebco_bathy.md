...
The output of `mar_read_gebco_bathy` is a matrix of class `bathy`, which
...
`mar_read_gebco_bathy` reads a 30 arcseconds or 1 arcminute bathymetry
file downloaded from the GEBCO (General Bathymetric Chart of the Oceans)
website (British Oceanographic Data Center). The website allows the
download of bathymetric data in the netCDF format.
`mar_read_gebco_bathy` uses the `ncdf4` package to load the data into R,
and parses it into an object of class `bathy`.
...
topography data. `mar_read_gebco_bathy` can read this type of database
when `sid` is set to `TRUE`. Then only the SID information will be
included in the object of class `bathy`. Therefore, to display a map
with both the bathymetry and the SID information, you will have to
...
`mar_get_noaa_bathy`, `mar_read_bathy`, `mar_plot_bathy`
