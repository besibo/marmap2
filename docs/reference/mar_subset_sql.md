...
`mar_subset_sql` queries the local SQL database created with
`mar_set_sql` to extract smaller data subsets.

## Usage

``` r
mar_set_sql(bathy, header = TRUE, sep = ",", db.name = "bathy_db")

setSQL(bathy, header = TRUE, sep = ",", db.name = "bathy_db")

mar_set_sql(bathy, header = TRUE, sep = ",", db.name = "bathy_db")
mar_subset_sql(min_lon, max_lon, min_lat, max_lat, db.name = "bathy_db")
...
## Arguments
  ...
  - db.name:
    ...
    `mar_set_sql` or from which `mar_subset_sql` will extract data
    ("bathy\_db" by default)
...
`mar_set_sql` returns `TRUE` if the database was successfully created.
`mar_subset_sql` returns a matrix of class `bathy` that can directly be
used with `mar_plot_bathy`.

## Details

Functions `mar_set_sql` and `mar_subset_sql` were built to work
together. `mar_set_sql` builds an SQL database and saves it on disk.
`mar_subset_sql` queries that local database and the fields `min_lon`,
`max_lon`, etc, are used to extract a subset of the database. The
functions were built as two entities so that multiple queries can be
done multiple times, without re-building the database each time. These
functions were designed to access the very large (\>5Go) ETOPO 2022 file
that can be downloaded from the NOAA website
...
mar_set_sql(bathy="NW_Atlantic.csv")

# uses data from the newly-created SQL database:
mar_subset_sql(min_lon=-70,max_lon=-50,
