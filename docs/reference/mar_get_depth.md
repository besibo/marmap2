...
``` r
mar_get_depth(mat, x, y=NULL, locator=TRUE, distance=FALSE, ...)
...
`mar_get_depth` allows the user to get depth data by clicking on a map
created with `mar_plot_bathy` or by providing coordinates of points of
interest. This function uses the `locator` function (`graphics`
package); after creating a map with `mar_plot_bathy`, the user can click
on the map once or several times (if `locator=TRUE`), press the Escape
...
point provided by GPS tracking devices. While `mar_get_transect` gets
every single depth value available in the bathymetric matrix between two
points along a user-defined transect, `mar_get_depth` only provides
depth data for the specific points provided as input by the user.

## See also

`mar_path_profile`, `mar_get_transect`, `mar_read_bathy`,
`mar_summary_bathy`, `mar_subset_bathy`, `nw.atlantic`
