...
`mar_lines_gc` draws Great Circle lines between a set of start and end
...
``` r
mar_lines_gc(start.points, end.points, n = 10, antimeridian = FALSE, ...)
...
`linesGCD` takes advantage of the `gcIntermediate` function from package
`geosphere` to plot lines following a great circle. When working with
`marmap` maps encompassing the antimeridian, longitudes are numbered
from 0 to 360 (as opposed to the classical numbering from -180 to +180).
It is thus critical to set `antimeridian=TRUE` to avoid plotting
incoherent great circle lines.

## See also

`mar_distance_to_isobath`, `mar_least_cost_distance`
