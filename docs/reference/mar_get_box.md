...
`mar_get_box` gets depth information of a belt transect of width `width`
around a transect defined by two points on a bathymetric map.

## Usage

``` r
mar_get_box(bathy,x1,x2,y1,y2,width,locator=FALSE,ratio=FALSE, ...)
...
## Arguments
  ...
  - locator:
    ...
    (`mar_plot_bathy(bathy,image=TRUE)`) is automatically plotted and
    the position of the belt transect is added.
...
`mar_get_box` allows the user to get depth data for a rectangle area of
the map around an approximate linear transect (belt transect). Both the
...
coordinates, or by clicking on a map created with `mar_plot_bathy`. In
its interactive mode, this function uses the `locator` function
(`graphics` package) to retrieve and plot the coordinates of the
selected transect. The argument `width` allows the user to specify the
width of the belt transect in degrees.

## See also

`mar_plot_bathy`, `mar_get_transect`, `mar_get_depth`
