...
`mar_plot_bathy`. When plotting collated data (with longitudes 0 to 180
and 180 to 360 degrees), plots can be modified to display the
conventional coordinate system (with longitudes 0 to 180 and -180 to 0
degrees) using function `mar_antimeridian_box()`.

## Details

This function is meant to be used with `mar_read_bathy()` or
`mar_read_gebco_bathy()`, when data is downloaded from either sides of
the antimeridian line (180 degrees longitude). If, for example, data is
downloaded from GEBCO for longitudes of 170E-180 and 180-170W,
`mar_collate_bathy()` will create a single matrix of class `bathy` with
a coordinate system going from 170 to 190 degrees longitude.

`mar_get_noaa_bathy()` deals with data from both sides of the
antimeridian and does not need further processing with
`mar_collate_bathy()`.

## See also

`mar_get_noaa_bathy`, `mar_summary_bathy`, `mar_plot_bathy`,
`mar_antimeridian_box`
