...
`plot_buffer` is a generic function that allows the plotting of objects
of class `buffer`, either as new plots or as a new layer added on top of
an existing one. The plotting of both the bathymetry/hypsometry as well
as the outline of the buffer is possible.

## Usage

``` r
plot_buffer(x, outline = TRUE, add = TRUE, ...)
```
...
## Arguments
  ...
  - x:
    ...
    an object of class `buffer` as produced by the `create_buffer()`
    function.
