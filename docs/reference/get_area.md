...
`get_area` calculates the projected surface area of specific depth
layers (e.g. upper bathyal, lower bathyal), the projected plane being
the ocean surface. The resolution of `get_area` depends on the
...
`plot_area`, `plot_bathy`, `contour`, `areaPolygon`
