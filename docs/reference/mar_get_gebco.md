...
## Arguments
  ...
  - path:
    ...
    `mar_get_gebco` looks for an already downloaded matching csv file.
...
`mar_get_gebco` uses the workflow implemented by the official GEBCO Grid
...
consistency with `mar_get_noaa`, but it is not sent to the GEBCO API:
the NetCDF file is downloaded at native resolution, then thinned locally
if `resolution` is larger than 0.25. Values smaller than 0.25 therefore
...
`mar_get_noaa`, `mar_read_gebco_bathy`, `mar_read_bathy`, `mar_as_bathy`
