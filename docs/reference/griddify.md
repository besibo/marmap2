...
## Arguments
  ...
  - x:
    ...
    three-column matrix, a point `sf` object, a `terra::SpatRaster`, or
    a historical `bathy` object.
  ...
  - crs:
    ...
    Coordinate reference system assigned to the target grid. Can be a
  ...
  - power:
    ...
    Numeric. Distance weighting power passed to `terra::interpIDW`.

  - class:
    
    Character. Output class. `"tbl"` returns a tibble with columns
    `lon`, `lat`, and `depth`; `"bathy"` returns a historical `bathy`
    matrix; `"spatraster"` returns a `terra::SpatRaster`.
...
Points falling in the same target cell are averaged. When `interpolate =
TRUE`, cells without observations are filled with inverse distance
weighting. This interpolation is intended as a pragmatic gridding step
for visualisation and exploratory mapping; it should not be interpreted
as a substitute for a domain-specific spatial interpolation model.

## See also

`as_bathy`, `as_spatraster`, `geom_bathy`, `quickplot_bathy`
