...
The output of `griddify` is an object of class `raster`, with `nlon`
unique longitude values and `nlat` unique latitude values.

## Details

`griddify` takes anys dataset with irregularly spaced xyz data and
transforms it into a raster object (i.e. a grid) with user specified
dimensions. `griddify` relies on several functions from the `raster`
...
`read_bathy`, `read_gebco_bathy`, `plot_bathy`
