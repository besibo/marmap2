...
`mar_plot_bathy` uses the base `contour` and `image` functions. If a
vector of isobath characteristics is provided, different types of
isobaths can be added to the same plot using a single call of
`mar_plot_bathy` (see examples)
...
then supplied as a vector to `mar_plot_bathy`) ; (3) colors can be
constrained to bathymetry- and/or topography. In this last case, a list
of vectors is supplied to `mar_plot_bathy` (example 7): each vector
