...
## Arguments
  ...
  - ...:
    ...
    2 or more buffer objects as produced by `create_buffer`. All `bathy`
    objects within the `buffer` objects must be compatible: they should
    have the same dimensions (same number of rows and columns) and cover
    the same area (same longitudes and latitudes).
...
`create_buffer`, `plot_buffer`, `plot_bathy`
