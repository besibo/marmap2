...
`mar_combine_buffers()`) in a format suitable for plotting its outline.
`mar_outline_buffer()` replaces any `NA` values in a `buffer` or `bathy`
...
buffer should be done using the `mar_plot_buffer()` function since it
offers a more straightforward method for plotting and much smoother
outlines, especially for low-resolution bathymetries.
