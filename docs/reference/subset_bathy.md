...
``` r
subset_bathy(mat, x, y=NULL, locator=TRUE, ...)
```
...
`subset_bathy` allows the user to generate new `bathy` objects by
...
polygon only. `subset_bathy` relies on the `point.in.polygon` function
from package `sp` to identify which points of the initial bathy matrix
lie witin the boundaries of the user-defined polygon.
...
`plot_bathy`, `get_depth`, `summary_bathy`, `aleutians`
